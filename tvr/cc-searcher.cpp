...
int main()
{
	...
	valarray<double> time_in_segment(num_areas);
	...
	valarray<double> area(num_areas);
        ...
        cout << "Enter number of searchers available: ";
        cin >> availableHours[0] ;

        for (i=0;i<num_areas;++i)
        {
            cout << endl 
                 << " Enter area in square miles for area number " << i 
                 << ": ";
            cin >> area[i];
            ...
            cout << " Enter time-in-segment for area number in hours " << i 
                 << ": ";
            cin >> time_in_segment[i] ;
        }
        ...
        for (i=0;i<num_areas;i++)
        {
            effectiveness[i][0]=ESW[i]*speed[i]*time_in_segment[i]/area[i];
        }
	...
	printAssignments(theAllocation);
        ...
        for (i=0;i<num_areas;i++)
        {
            cout << "  " << i << ": Old: " << POA[i]*ESW[i]*speed[i]*time_in_segment[i]/area[i]
                 << " New: " 
                 << theAllocation.getNewPOC(i)*ESW[i]*speed[i]*time_in_segment[i]/area[i]
                 << endl;
        }
            ...
            if (ans == "yes") 
            {
                ...
                while(!(activeItr.atEnd()))
                {
                    ...
                    while (!(resItr.atEnd()))
                    {
                        ...
                        cout << " Change number searchers? " << endl;
                        cin >> ans;
                        if (ans == "yes")
                        {
                            double new_time;
                            cout << " New number searchers: ";
                            cin >> new_time;
                ...
                for (i=0;i<num_areas;i++)
                    {
                        cout << "  " << i << ": Old: " 
                             << POA[i]*ESW[i]*speed[i]*time_in_segment[i]/area[i]
                             << " New: " 
                             << changedAllocation.getNewPOC(i)*ESW[i]*speed[i]*time_in_segment[i]/area[i]
                             << endl;
                    }
                cout << "Segment POSs: " << endl;
                for (i=0;i<num_areas;i++)
